...
export function writeQueryEnv(writer: Write, type: QueryEnv): void {
  ...
  writer.context().push("queryStr", "string | null", "writing property");
  writer.writeString("queryStr");
  writer.writeNullableString(type.queryStr);
  writer.context().pop();
...
export function readQueryEnv(reader: Read): QueryEnv {
  ...
  let _queryStr: string | null = null;
  let _str: string = "";
  ...
  while (numFields > 0) {
    ...
    if (field == "queryStr") {
      ...
      _queryStr = reader.readNullableString();
      reader.context().pop();
    }
    else if (field == "str") {

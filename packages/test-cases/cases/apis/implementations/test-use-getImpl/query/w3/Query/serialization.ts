...
export class Input_queryImplementations {
}

export function deserializequeryImplementationsArgs(argsBuf: ArrayBuffer): Input_queryImplementations {
  const context: Context =  new Context("Deserializing module-type: queryImplementations");

  return {
  };
}

export function serializequeryImplementationsResult(result: Array<string>): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) module-type: queryImplementations");
  const sizer = new WriteSizer(sizerContext);
  writequeryImplementationsResult(sizer, result);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) module-type: queryImplementations");
  const encoder = new WriteEncoder(buffer, sizer, encoderContext);
  writequeryImplementationsResult(encoder, result);
  return buffer;
}

export function writequeryImplementationsResult(writer: Write, result: Array<string>): void {
  writer.context().push("queryImplementations", "Array<string>", "writing property");

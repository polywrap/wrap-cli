...
export function deserializeabstractMutationMethodArgs(argsBuf: ArrayBuffer): Input_abstractMutationMethod {
  const context: Context =  new Context("Deserializing module-type: abstractMutationMethod");
  const reader = new ReadDecoder(argsBuf, context);
  let numFields = reader.readMapLength();
...
export function serializeabstractMutationMethodResult(result: u8): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) module-type: abstractMutationMethod");
  const sizer = new WriteSizer(sizerContext);
  writeabstractMutationMethodResult(sizer, result);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) module-type: abstractMutationMethod");
  const encoder = new WriteEncoder(buffer, sizer, encoderContext);
  writeabstractMutationMethodResult(encoder, result);
  return buffer;
}

export function writeabstractMutationMethodResult(writer: Write, result: u8): void {
  writer.context().push("abstractMutationMethod", "u8", "writing property");
  writer.writeUInt8(result);
  writer.context().pop();
}

...
export function _w3_invoke(method_size: u32, args_size: u32): bool {
  ...
  if (args.method == "sanitizeEnv") {
    return w3_invoke(args, sanitizeEnvWrapped);
  }
  else if (args.method == "mutEnvironment") {
    return w3_invoke(args, mutEnvironmentWrapped);
  }
  else {
...
export function _w3_sanitize_env(args_size: u32): void {
  w3_sanitize_env(args_size, sanitizeEnvWrapped)
}

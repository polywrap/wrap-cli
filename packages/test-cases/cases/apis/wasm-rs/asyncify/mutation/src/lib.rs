...
pub fn set_data_with_many_structured_args(input: mutation::InputSetDataWithManyStructuredArgs) -> bool {
    let _ = EthereumMutation::call_contract_method(&ethereum_mutation::InputCallContractMethod {
        address: input.address,
        method: "function set(value: u64)".to_string(),
...
pub fn deploy_contract(input: mutation::InputDeployContract) -> String {
    ...
    EthereumMutation::deploy_contract(&ethereum_mutation::InputDeployContract {
        abi,
        bytecode,
        args: None,
        connection: input.connection,

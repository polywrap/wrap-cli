...
pub fn write_test_import_object<W: Write>(input: &TestImportObject, writer: &mut W) {
    ...
    writer
        .context()
        .push("object", "Box<TestImportAnotherObject>", "writing property");
    writer.write_str("object");
    TestImportAnotherObject::write(&input.object, writer);
    writer.context().pop();
    writer.context().push(
        "opt_object",
        "Option<Box<TestImportAnotherObject>>",
        "writing property",
    );
    ...
    writer.context().push(
        "object_array",
        "Vec<Box<TestImportAnotherObject>>",
        "writing property",
    );
    ...
    writer.context().push(
        "opt_object_array",
        "Option<Vec<Option<Box<TestImportAnotherObject>>>>",
        "writing property",
    );
...
pub fn read_test_import_object<R: Read>(reader: &mut R) -> Result<TestImportObject, String> {
    ...
    Ok(TestImportObject {
        object: match _object {
            Some(x) => return x,
            None => panic!(
                "'object' is required, but its value is still 'None'. This should never happen."
            ),
        },

...
pub fn write_custom_type<W: Write>(args: &CustomType, writer: &mut W) -> Result<(), EncodeError> {
    ...
    writer.write_nullable_bigint(&args.opt_bigint)?;
    writer.context().pop();
    ...
    writer.write_nullable_bignumber(&args.opt_bignumber)?;
    writer.context().pop();
    ...
    writer.write_nullable_json(&args.opt_json)?;
    writer.context().pop();
    ...
    writer.write_nullable_bytes(&args.opt_bytes)?;
    writer.context().pop();
    ...
    writer.write_nullable_bool(&args.opt_boolean)?;
    writer.context().pop();
    ...
    writer.write_nullable_array(&args.u_opt_array, |writer, item| {
        writer.write_u32(item)
    })?;
    writer.context().pop();
    writer.context().push("optUOptArray", "Option<Vec<Option<u32>>>", "writing property");
    writer.write_string("optUOptArray")?;
    writer.write_nullable_array(&args.opt_u_opt_array, |writer, item| {
    ...
    writer.write_nullable_array(&args.opt_str_opt_array, |writer, item| {
    ...
    writer.write_nullable_array(&args.crazy_array, |writer, item| {
        writer.write_nullable_array(item, |writer, item| {
            writer.write_array(item, |writer, item| {
                writer.write_nullable_array(item, |writer, item| {
                    writer.write_u32(item)

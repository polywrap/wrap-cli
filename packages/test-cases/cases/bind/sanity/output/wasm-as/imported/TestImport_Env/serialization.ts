...
export function serializeEnv(type: TestImport_Env): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) env-type: TestImport_Env");
  const sizer = new WriteSizer(sizerContext);
  writeTestImport_Env(sizer, type);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) env-type: TestImport_Env");
...
export function readTestImport_Env(reader: Read): TestImport_Env {
  ...
  let _enviroPropSet: boolean = false;

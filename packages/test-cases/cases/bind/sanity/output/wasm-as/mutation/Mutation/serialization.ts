...
export function deserializemutationMethodArgs(
  argsBuf: ArrayBuffer
): Input_mutationMethod {
  const context: Context = new Context(
    "Deserializing query-type: mutationMethod"
  );
  ...
  if (!_strSet) {
    throw new Error(
      reader
        .context()
        .printWithContext("Missing required argument: 'str: String'")
    );
  }
  if (!_enSet) {
    throw new Error(
      reader
        .context()
        .printWithContext("Missing required argument: 'en: CustomEnum'")
    );
  }
  if (!_enumArraySet) {
    throw new Error(
      reader
        .context()
        .printWithContext(
          "Missing required argument: 'enumArray: [CustomEnum]'"
        )
    );
  }
...
export function serializemutationMethodResult(result: i32): ArrayBuffer {
  const sizerContext: Context = new Context(
    "Serializing (sizing) query-type: mutationMethod"
  );
  const sizer = new WriteSizer(sizerContext);
  writemutationMethodResult(sizer, result);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context(
    "Serializing (encoding) query-type: mutationMethod"
  );
...
export function writemutationMethodResult(writer: Write, result: i32): void {
  writer.context().push("mutationMethod", "i32", "writing property");
...
export function deserializeobjectMethodArgs(
  ...
  const context: Context = new Context(
    "Deserializing query-type: objectMethod"
  );
  const reader = new ReadDecoder(argsBuf, context);
  let numFields = reader.readMapLength();
  ...
  if (!_object || !_objectSet) {
    throw new Error(
      reader
        .context()
        .printWithContext("Missing required argument: 'object: AnotherType'")
    );
  }
  if (!_objectArraySet) {
    throw new Error(
      reader
        .context()
        .printWithContext(
          "Missing required argument: 'objectArray: [AnotherType]'"
        )
    );
  }
...
export function serializeobjectMethodResult(
  ...
  const sizerContext: Context = new Context(
    "Serializing (sizing) query-type: objectMethod"
  );
  const sizer = new WriteSizer(sizerContext);
  writeobjectMethodResult(sizer, result);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context(
    "Serializing (encoding) query-type: objectMethod"
  );
  const encoder = new WriteEncoder(buffer, encoderContext);
...
export function writeobjectMethodResult(
  ...
  writer
    .context()
    .push("objectMethod", "Types.AnotherType | null", "writing property");
  if (result) {

...
export function writeimportedMethodArgs(
  ...
  writer
    .context()
    .push("object", "Types.TestImport_Object", "writing property");
  writer.writeString("object");
  Types.TestImport_Object.write(writer, input.object);
  writer.context().pop();
  writer
    .context()
    .push("optObject", "Types.TestImport_Object | null", "writing property");
  writer.writeString("optObject");
  if (input.optObject) {
    Types.TestImport_Object.write(
      writer,
      input.optObject as Types.TestImport_Object
    );
  } else {
    writer.writeNil();
  }
  ...
  writer
    .context()
    .push("objectArray", "Array<Types.TestImport_Object>", "writing property");
  writer.writeString("objectArray");
  writer.writeArray(
    input.objectArray,
    (writer: Write, item: Types.TestImport_Object): void => {
  ...
  writer
    ...
    .push(
      "optObjectArray",
      "Array<Types.TestImport_Object | null> | null",
      "writing property"
    );
  writer.writeString("optObjectArray");
  writer.writeNullableArray(
    input.optObjectArray,
    (writer: Write, item: Types.TestImport_Object | null): void => {
      if (item) {
...
export function deserializeimportedMethodResult(
  buffer: ArrayBuffer
): Types.TestImport_Object | null {
  const context: Context = new Context(
    "Deserializing imported query-type: importedMethod"
  );
...
export function serializeanotherMethodArgs(
  ...
  const sizerContext: Context = new Context(
    "Serializing (sizing) imported query-type: anotherMethod"
  );
  const sizer = new WriteSizer(sizerContext);
  writeanotherMethodArgs(sizer, input);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context(
    "Serializing (encoding) imported query-type: anotherMethod"
  );
  const encoder = new WriteEncoder(buffer, encoderContext);
...
export function deserializeanotherMethodResult(buffer: ArrayBuffer): i32 {
  const context: Context = new Context(
    "Deserializing imported query-type: anotherMethod"
  );
  const reader = new ReadDecoder(buffer, context);

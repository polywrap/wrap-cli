...
import wasmer
from wasm.types import W3Imports, W3ImportsW3
...
class W3ImportW3Implementation(W3ImportsW3):
    ...
    async def __wrap_subinvoke(
        ...
        args = read_bytes(cls.memory.buffer, args_ptr, args_len)
    ...
    def __wrap_subinvoke_result_len(cls) -> int:
        ...
        if not cls.state.subinvoke.result:
            cls.abort("__wrap_subinvoke_result_len: subinvoke.result is not set")
            return 0
    ...
    def __wrap_subinvoke_result(cls, ptr: int):
        ...
        if not cls.state.subinvoke.result:
            cls.abort("__wrap_subinvoke_result: subinvoke.result is not set")
            return
    ...
    def __wrap_subinvoke_error_len(cls) -> int:
        ...
        if not cls.state.subinvoke.error:
            cls.abort("__wrap_subinvoke_error_len: subinvoke.error is not set")
            return 0
    ...
    def __wrap_subinvoke_error(cls, ptr: int):
        ...
        if not cls.state.subinvoke.error:
            cls.abort("__wrap_subinvoke_error: subinvoke.error is not set")
            return
    ...
    async def __wrap_subinvoke_implementation(
        ...
        cls.state.subinvoke_implementation.args = [impl_uri, method, args]
    ...
    def __wrap_subinvoke_implementation_result_len(cls) -> int:
        if not cls.state.subinvoke_implementation.result:
            cls.abort("__wrap_subinvokeImplementation_result_len: subinvokeImplementation.result is not set")
            return 0
    ...
    def __wrap_subinvoke_implementation_result(cls, ptr: int):
        if not cls.state.subinvoke_implementation.result:
            cls.abort("__wrap_subinvokeImplementation_result: subinvokeImplementation.result is not set")
            return
    ...
    def __wrap_subinvoke_implementation_error_len(cls) -> int:
        if not cls.state.subinvoke_implementation.error:
            cls.abort("__wrap_subinvokeImplementation_error_len: subinvokeImplementation.error is not set")
            return 0
    ...
    def __wrap_subinvoke_implementation_error(cls, ptr: int):
        if not cls.state.subinvoke_implementation.error:
            cls.abort("__wrap_subinvokeImplementation_error: subinvokeImplementation.error is not set")
            return
    ...
    def __wrap_invoke_args(cls, method_ptr: int, args_ptr: int):
        ...
        if not cls.state.method:
            cls.abort("__wrap_invoke_args: method is not set")
            return
        if not cls.state.args:
            cls.abort("__wrap_invoke_args: args is not set")
            return
    ...
    @classmethod
    def __wrap_get_implementations(cls, uri_ptr: int, uri_len: int):
        uri = read_string(cls.memory.buffer, uri_ptr, uri_len)
    ...
    @classmethod
    def __wrap_get_implementations_result_len(cls) -> int:
        if not cls.state.get_implementation_result:
            cls.abort("__wrap_getImplementations_result_len: result is not set")
            return 0
        
        return cls.state.get_implementations_result.byteLength

    @classmethod
    def __wrap_get_implementations_result(cls, ptr: int):
        if not cls.state.get_implementations_result:
            cls.abort("__wrap_getImplementations_result: result is not set")
            return
    ...
    def __wrap_sanitize_env_args(cls, ptr: int):
        if not cls.state.sanitize_env.args:
            cls.abort("__wrap_sanitize_env: args is not set")
            return
...
def create_imports(
    ...
    return W3Imports(
        wrap = W3ImportW3Implementation(config),
        env = config["memory"]
    )

...
from . import UriResolverWrapper
...
class ExtendableUriResolver(UriResolver):
    ...
    async def resolve_uri(
        ...
        if not self._has_loaded_uri_resolvers:
            success, failed_uri_resolvers = await self.load_uri_resolver_wrappers(
                client, cache, uri_resolver_impls
            )
    ...
    async def load_uri_resolver_wrappers(
        ...
        bootstrap_uri_resolvers = list(
            filter(lambda x: x.name != self.name, client.get_uri_resolvers({}))
        )
        ...
        while implementation_uri == implementations_to_load.get():
            ...
            api = await client.resolve_uri(
                implementation_uri,
                {"config": {"uri_resolvers": bootstrap_uri_resolvers}},
            )
            ...
            if not api:
                ...
                if failed_attempts == len(implementations_to_load):
                    return {
                        "success": False,
                        "failed_uri_resolvers": list(
                            x.uri for x in implementations_to_load
                        ),
                    }
                else:
    ...
    async def _create_uri_resolver_wrappers(
        ...
        for implementation_uri in implementation_uris:
            uri_resolver_impl = UriResolverWrapper(
                implementation_uri, self._create_api, self._deserialize_options
            )

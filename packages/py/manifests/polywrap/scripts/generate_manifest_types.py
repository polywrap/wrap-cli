...
"""
INPUT_PATH: Final[Path] = Path('../../../manifest-schemas/formats/')
OUTPUT_PATH: Final[Path] = Path('../../src/core/manifest/formats/')

if __name__ == '__main__':
    for directory_path, _, file_names in os.walk(INPUT_PATH):
        schemaType = Path(directory_path).parts[-1]
        if schemaType == 'formats':
            continue

        formatModules = []
        schemata = dict()
        for file in file_names:
            schemaVersion = file.split('.json')[0]
            try:
                schema = materialize(
                ...
                schemata[f'{schemaType}@{schemaVersion}'] = parse(schema)

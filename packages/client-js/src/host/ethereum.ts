...
export interface IEthereumImports {
  _w3_eth_callView(
    addressPtr: number, methodPtr: number,
    argsPtr: number, cb: WasmCallback
  ): Promise<void /*string*/>;
  ...
  _w3_eth_deployContract(
    abiPtr: number, bytecodePtr: number, cb: WasmCallback
  ): Promise<void /*string*/>;
}

export function getEthImports(
  getWasmWorker: () => WasmWorker,
  ethereum: Ethereum
): IEthereumImports {
  return {
    _w3_eth_callView: async (
      addressPtr: number, methodPtr: number,
      argsPtr: number, cb: WasmCallback
    ) => {

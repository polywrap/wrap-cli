...
// pub fn from_tx_receipt(receipt: EthereumTxReceipt) -> TransactionReceipt {
//     TransactionReceipt {
...
// pub fn from_tx_request(request: EthereumTxRequest) -> TransactionRequest {
//     TransactionRequest {
...
//         chain_id: request.chain_id.map(|f| U64::try_from(f).unwrap()),
//     }
// }

// pub fn to_log(log: EthersLog) -> EthereumLog {
//   EthereumLog {
...
// pub fn from_log(log: EthereumLog) -> EthersLog {
//     EthersLog {
...
// fn get_polywrapper_logs(receipt: &TransactionReceipt) -> Vec<EthereumLog> {
//     let mut logs: Vec<EthereumLog> = vec![];
//     for log in receipt.logs.clone() {
//         logs.push(EthereumLog {
//             address: log.address.to_string(),
//             topics: log.topics.into_iter().map(|f| f.to_string()).collect(),
//             data: String::from_utf8(log.data.0.to_vec()).unwrap(),
//             block_hash: log.block_hash.map(|f| f.to_string()),
...
// fn get_ethers_core_logs(receipt: &EthereumTxReceipt) -> Vec<EthersLog> {

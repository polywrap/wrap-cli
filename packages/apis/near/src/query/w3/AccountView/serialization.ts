...
export function serializeAccountView(type: AccountView): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) object-type: AccountView");
  const sizer = new WriteSizer(sizerContext);
  writeAccountView(sizer, type);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) object-type: AccountView");
  const encoder = new WriteEncoder(buffer, encoderContext);
  writeAccountView(encoder, type);
  return buffer;
}

export function writeAccountView(writer: Write, type: AccountView): void {
...
export function deserializeAccountView(buffer: ArrayBuffer): AccountView {
  const context: Context = new Context("Deserializing object-type AccountView");
  const reader = new ReadDecoder(buffer, context);
  return readAccountView(reader);
}

export function readAccountView(reader: Read): AccountView {

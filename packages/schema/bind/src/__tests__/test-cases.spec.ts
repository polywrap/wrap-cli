...
import { bindSchema, BindLanguage, BindOutput } from "../";
    ...
    it(`Case: ${test.name}`, async () => {
      ...
      for (const outputLanguage of testCase.outputLanguages) {
          ...
          for (const moduleName of Object.keys(directories.moduleWise)) {
            if (moduleName === "common") {
              expectedModuleWiseOutput.common = {
                name: moduleName,
                output: readDirectorySync(directories.moduleWise[moduleName]),
        ...
        const hasCommonModule =
          Object.keys(directories.moduleWise || {}).indexOf("common") > -1;
        ...
        const validateOutput = (
          ...
          if (expected.common) {
            expected.common.output.entries = sort(
              expected.common.output.entries
            );
          }
          ...
          writeFileSync(
            path.join(
              testResultDir,
              tag ? `${language}-${tag}-output.json` : `${language}-output.json`
            ),

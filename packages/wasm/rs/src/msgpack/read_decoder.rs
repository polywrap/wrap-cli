...
impl ReadDecoder {
	...
	fn get_size(&mut self) -> Result<i32, String> {
		...
		if Format::is_negative_fixed_int(lead_byte) || Format::is_fixed_int(lead_byte) {
			...
			match lead_byte {
				...
				_ => {
					return Err(
						["invalid prefix, bad encoding for val: ", &lead_byte.to_string()].concat()
					)
				},
			}
		}
	...
	fn get_error_message(lead_byte: u8) -> Result<&'static str, String> {
		if Format::is_negative_fixed_int(lead_byte) || Format::is_fixed_int(lead_byte) {
			...
			match lead_byte {
				...
				_ =>
					Err(["invalid prefix, bad encoding for val: {}", &lead_byte.to_string()]
						.concat()),
			}
		}
	}
			...
			Format::UINT64 => {
				...
				if value <= i64::MAX as u64 {
					...
					Err(self.context.print_with_context(
						&["Integer overflow: value = ", &value.to_string(), "; bits = 64"].concat(),
					))
				}
			},
			_ => Err(self.context.print_with_context(
				&["Property must be of type `int`", Self::get_error_message(prefix)?].concat(),
			)),
		}
	}
	...
	fn read_u64(&mut self) -> Result<u64, String> {
		...
		if Format::is_fixed_int(prefix) {
			...
			return Err(
				["unsigned integer cannot be negative: prefix = ", &prefix.to_string()].concat()
			)
		}
		match prefix {
			...
			_ => Err(self.context.print_with_context(
				&["Property must be of type `uint`", Self::get_error_message(prefix)?].concat(),
			)),
		}
	}
}
...
impl Read for ReadDecoder {
	fn read_bool(&mut self) -> Result<bool, String> {
		...
		Err(self.context.print_with_context(
			&["Property must be of type `bool`", Self::get_error_message(value)?].concat(),
		))
	}
	...
	fn read_i16(&mut self) -> Result<i16, String> {
		...
		Err(self.context.print_with_context(
			&["integer overflow: value = ", &value.to_string(), "bits = 16"].concat(),
		))
	}
	...
	fn read_i32(&mut self) -> Result<i32, String> {
		...
		Err(self.context.print_with_context(
			&["integer overflow: value = ", &value.to_string(), "bits = 32"].concat(),
		))
	}
	...
	fn read_u8(&mut self) -> Result<u8, String> {
		...
		Err(self.context.print_with_context(
			&["unsigned integer overflow: value = ", &value.to_string(), "bits = 8"].concat(),
		))
	}
	...
	fn read_u16(&mut self) -> Result<u16, String> {
		...
		Err(self.context.print_with_context(
			&["unsigned integer overflow: value = ", &value.to_string(), "bits = 16"].concat(),
		))
	}
	...
	fn read_u32(&mut self) -> Result<u32, String> {
		...
		Err(self.context.print_with_context(
			&["unsigned integer overflow: value = ", &value.to_string(), "bits = 32"].concat(),
		))
	}
	...
	fn read_f32(&mut self) -> Result<f32, String> {
		...
		Err(self.context.print_with_context(
			&["Property must be of type `float32`", Self::get_error_message(prefix)?].concat(),
		))
	}
	...
	fn read_f64(&mut self) -> Result<f64, String> {
		...
		Err(self.context.print_with_context(
			&["Property must be of type `float64`", Self::get_error_message(prefix)?].concat(),
		))
	}
	...
	fn read_string_length(&mut self) -> Result<u32, String> {
		...
		match lead_byte {
			...
			_ => Err(self.context.print_with_context(
				&["Property must be of type `string`", Self::get_error_message(lead_byte)?]
					.concat(),
			)),
		}
	}
	...
	fn read_bytes_length(&mut self) -> Result<u32, String> {
		...
		match lead_byte {
			...
			Format::STR32 => Ok(self.view.get_u32()),
			_ => Err(self.context.print_with_context(
				&["Property must be of type `bytes`", Self::get_error_message(lead_byte)?].concat(),
			)),
		}
	}
	...
	fn read_array_length(&mut self) -> Result<u32, String> {
		...
		Err(self.context.print_with_context(
			&["Property must be of type `array`", Self::get_error_message(lead_byte)?].concat(),
		))
	}
	...
	fn read_map_length(&mut self) -> Result<u32, String> {
		...
		match lead_byte {
			...
			Format::NIL => Ok(0),
			_ => Err(self.context.print_with_context(
				&["Property must be of type `map`", Self::get_error_message(lead_byte)?].concat(),
			)),
		}
	}

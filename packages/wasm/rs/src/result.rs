...
impl wasm_result {
	...
	pub const OK: Self =
		Self { receipt: std::ptr::null_mut(), error: std::ptr::null_mut(), buf_size: 0 };
	...
	pub fn receipt(&self) -> Option<&[u8]> {
		if !self.receipt.is_null() {
			unsafe { Some(std::slice::from_raw_parts(self.receipt, self.buf_size as usize)) }
		} else {
			None
		}
	}
...
mod test {
	...
	#[test]
	fn test_receipt() {
		assert_eq!(wasm_result::new_receipt(b"foobar").receipt().unwrap(), b"foobar");
	}
}

...
extern "C" {
    /// Subinvoke API
    #[wasm_bindgen(js_name = __w3_subinvoke)]
    ...
    /// Subinvoke Result
    #[wasm_bindgen(js_name = __w3_subinvoke_result_len)]
    pub fn __w3_subinvoke_result_len() -> u32;

    #[wasm_bindgen(js_name = __w3_subinvoke_result)]
    pub fn __w3_subinvoke_result(ptr: u32);

    /// Subinvoke Error
    #[wasm_bindgen(js_name = __w3_subinvoke_error_len)]
    pub fn __w3_subinvoke_error_len() -> u32;

    #[wasm_bindgen(js_name = __w3_subinvoke_error)]
...
pub fn w3_subinvoke(
    ...
    if !success {
        let error_len = __w3_subinvoke_error_len();
        let error_len_ptr = alloc(error_len as usize);
        __w3_subinvoke_error(error_len_ptr as u32);
        let error = unsafe {
            String::from_raw_parts(error_len_ptr, error_len as usize, error_len as usize)
        };
        return Err(error);
    }
    let result_len = __w3_subinvoke_result_len();
    let result_len_ptr = alloc(result_len as usize);
    __w3_subinvoke_result(result_len_ptr as u32);
    let result_buf =

...
fn serialize_sanity<W: Write>(mut writer: W, sanity: &mut Sanity) {
    ...
    writer.write_nullable_string(sanity.nil.clone());
    ...
    writer.write_string(sanity.string.clone());
    ...
    writer.write_bytes(sanity.bytes.clone());
    writer.write_string("large_bytes".to_string());
    writer.write_bytes(sanity.large_bytes.clone());

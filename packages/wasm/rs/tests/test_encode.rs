...
fn test_write_f32() {
	...
	assert_eq!([0xca, 0xff, 0x7f, 0xff, 0xff], writer.get_buffer().as_slice());
}

#[test]
fn test_write_f64() {
	let mut writer =
		WriteEncoder::new(&[0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], Context::new());
...
fn test_write_i32() {
	...
	assert_eq!([0xd2, 0x80, 0x00, 0x00, 0x00], writer.get_buffer().as_slice());
}
...
fn test_write_u32() {
	...
	assert_eq!([0xce, 0xff, 0xff, 0xff, 0xff], writer.get_buffer().as_slice());
}

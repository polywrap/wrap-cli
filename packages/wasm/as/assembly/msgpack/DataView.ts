...
export class DataView {
  ...
  constructor(buffer: ArrayBuffer, byte_offset: i32 = 0, byte_length: i32 = buffer.byteLength) {
    if (i32(<u32>byte_length > <u32>BLOCK_MAXSIZE) | i32(<u32>byte_offset + byte_length > <u32>buffer.byteLength))
  ...
  getBytes(length: i32): ArrayBuffer {
    if (this.byteOffset + length > this.byteLength) throw new RangeError(E_INDEXOUTOFRANGE);
  ...
  setBytes(buf: ArrayBuffer): void {
    if (this.byteOffset + buf.byteLength > this.byteLength) throw new RangeError(E_INDEXOUTOFRANGE);
    memory.copy(changetype<i32>(this.dataStart) + this.byteOffset, changetype<i32>(buf), buf.byteLength);
    this.byteOffset += buf.byteLength;
  }

  peekUint8(): u8 {
    if (this.byteOffset > this.byteLength) throw new RangeError(E_INDEXOUTOFRANGE);
  ...
  getFloat32(): f32 {
    if (this.byteOffset + 4 > this.byteLength) throw new RangeError(E_INDEXOUTOFRANGE);
  ...
  getFloat64(): f64 {
    if (this.byteOffset + 8 > this.byteLength) throw new RangeError(E_INDEXOUTOFRANGE);
  ...
  getInt16(): i16 {
    if (this.byteOffset + 2 > this.byteLength) throw new RangeError(E_INDEXOUTOFRANGE);

...
describe("Polywrap Workflow Validation", () => {
  it("Should throw incorrect version format error", async () => {
    const workflowPath = __dirname + "/manifest/workflow/incorrect-version-format/polywrap.test.yaml";
  ...
  it("Should throw not accepted field error", async () => {
    const workflowPath = __dirname + "/manifest/workflow/not-accepted-field/polywrap.test.yaml";
    const workflow = fs.readFileSync(workflowPath, "utf-8");
  ...
  it("Should throw required field missing error", async () => {
    const workflowPath = __dirname + "/manifest/workflow/required-field-missing/polywrap.test.yaml";
    const workflow = fs.readFileSync(workflowPath, "utf-8");
  ...
  it("Should throw wrong type error", async () => {
    const workflowPath = __dirname + "/manifest/workflow/wrong-type/polywrap.test.yaml";
    const workflow = fs.readFileSync(workflowPath, "utf-8");

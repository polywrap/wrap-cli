import { deserializePolywrapManifest } from "../manifest";
...
describe("Polywrap Manifest Validation", () => {
  it("Should throw file string does not exist error", async () => {
    ...
    expect(() => deserializePolywrapManifest(manifest)).toThrowError(/instance.schema does not conform to the "graphqlFile" format/);
  });

...
export const findPluginPackage = Tracer.traceFunc(
  ...
  (uri: Uri, redirects: readonly UriRedirect<Uri>[]): PluginPackage | undefined => {
    const pluginRedirect = redirects.find(redirect => Uri.equals(redirect.from, uri) && !Uri.isUri(redirect.to));
